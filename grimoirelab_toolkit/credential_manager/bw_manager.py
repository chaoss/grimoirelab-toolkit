...
class BitwardenManager(CredentialManager):
    ...
    parallel without sharing session state. The system environment
    variables are inherited so that the Bitwarden CLI can locate
    its dependencies (e.g. ``node``). The temporary directory is
    cleaned up automatically when manager.logout is called.
